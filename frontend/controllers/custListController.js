...
var populateCustomerList = function(req, res){
	...
	request({url:'http://customer:9000/getCustomers',qs:query_params}, function (error, response, body) {
      if (!error) { 
		res.send(body);
	  }	
	  else {
...
var populateCustomerDetails = function(req, res){
	...
	request('http://customer-detail:9000/getCustomerDetails/' + req.params.custid, function (error, response, body) {
      if (!error) {
		res.send(body);
	  }	
	  else {
